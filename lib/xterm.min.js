(function(){
function __xtermSplitRules(css){
  var rules=[],depth=0,start=0;
  for(var i=0;i<css.length;i++){
    if(css[i]==='{')depth++;
    else if(css[i]==='}'){depth--;if(depth===0){rules.push(css.slice(start,i+1).trim());start=i+1;}}
  }
  return rules.filter(function(r){return r.length>0;});
}
window.__xtermStyleProxy=function(){
  var lnk=document.getElementById('xterm-dyn');
  if(!lnk||!lnk.sheet)return null;
  var sh=lnk.sheet,n=0;
  return{set textContent(c){
    while(n>0){try{sh.deleteRule(0);}catch(e){}n--;}
    if(!c)return;
    var rules=__xtermSplitRules(c);
    for(var i=0;i<rules.length;i++){try{sh.insertRule(rules[i],sh.cssRules.length);n++;}catch(e){}}
  }};
};
}());
/**
